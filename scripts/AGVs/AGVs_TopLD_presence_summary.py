...
def AGVs_TopLD_presence_summary(AGVs_BED, chr, TopLD_directory, AGV_LD_variants_directory, output):
	...
	1) population presence in TopLD
	2) MAFs for population presence
	3) populations where the variant occurs in LD with at least one other variant
	"""
	...
	for population in populations:
		annotation_path = os.path.join(TopLD_directory, f'{population}_{chr}_no_filter_0.2_1000000_info_annotation.csv.gz')

		if not os.path.isfile(annotation_path):
			print(f"Warning: File '{annotation_path}' does not exist. Skipping population {population} for chromosome {chr}.")
			continue
		...
		TopLD_populations = merged_df.dropna(subset=['MAF'])

		LD_file_path = os.path.join(AGV_LD_variants_directory, f'{population}_{chr}_AGV_LD_variants.txt.gz')
		...
		if os.path.isfile(LD_file_path):
			...
			matching_AGVs = set(TopLD_populations['ID'].values) & AGVs_in_LD

			if matching_AGVs:
				LD_populations.append(population)

		for _, row in TopLD_populations.iterrows():
			variant_key = (row['chr'], row['end'])

			if variant_key not in results_dict:
				results_dict[variant_key] = {
					'populations_present_in_TopLD': [],
					'TopLD_MAFs': [],
					'populations_with_LDVs': []
				}

			rounded_MAF = round(row['MAF'], 3)

			results_dict[variant_key]['populations_present_in_TopLD'].append(population)
			results_dict[variant_key]['TopLD_MAFs'].append(str(rounded_MAF))
			results_dict[variant_key]['populations_with_LDVs'].extend(LD_populations)
	...
	with open(output, 'w') as out:
		for (chr, pos), data in results_dict.items():
			...
			population_order = ['AFR', 'EAS', 'EUR', 'SAS']
			sorted_populations = sorted(set(data['populations_present_in_TopLD']), key=lambda x: population_order.index(x))
			sorted_MAFs = [data['TopLD_MAFs'][data['populations_present_in_TopLD'].index(pop)] for pop in sorted_populations]
			sorted_LD_pops = sorted(set(data['populations_with_LDVs']), key=lambda x: population_order.index(x))

			out.write(f"{join_ID}\t{','.join(sorted_populations)}\t{','.join(sorted_MAFs)}\t{','.join(sorted_LD_pops)}\n")
