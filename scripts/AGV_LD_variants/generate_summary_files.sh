...
#$ -m ae
#$ -cwd
#$ -o /wynton/group/capra/projects/AADR_proxy_variants/scripts/AGV_LD_variants/generate_summary_files.out
#$ -e /wynton/group/capra/projects/AADR_proxy_variants/scripts/AGV_LD_variants/generate_summary_files.err
#$ -l h_rt=12:00:00
#$ -l mem_free=50G
...
# run
python3 "$script" --AGV_LD_variants_directory "$AGV_LD_variants_directory" --TopLD_directory "$TopLD_directory" --chromosome "$chr" --out "${out_directory}${chr}_summary.txt.gz"
