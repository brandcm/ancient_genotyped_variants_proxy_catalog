...
pop_chrs="${project_directory}data/metadata/chrs_by_pop.txt"
pop=$(awk -v row=$SGE_TASK_ID 'NR == row {print $1}' "$pop_chrs")
chr=$(awk -v row=$SGE_TASK_ID 'NR == row {print $2}' "$pop_chrs")
dictionary="${project_directory}data/1240K_variants/1240K_variants_hg38.pkl"
...
python3 "$script" --dictionary "$dictionary" --population "$pop" --chromosome "$chr" --LD_files_directory "$TopLD_directory" --out_directory "$out_directory"
