...
def parse_args():
	...
	parser.add_argument('--population', type=str, required=True, help='Population ID to analyze (AFR, EAS, EUR, or SAS).')
...
def main():
	...
	MAF_class_sums = sum_TopLD_variants_per_MAF_class(TopLD_directory_path, pop)
	if not MAF_class_sums:
		print("Error: Could not calculate MAF class counts.")
		return

	proportions_df = count_TopLD_variants_per_LD_threshold_and_MAF_class(pop, TopLD_directory_path, LD_directory_path, MAF_classes, MAF_class_sums, LD_thresholds)
	proportions_df.to_csv(args.out, sep='\t', header=True, index=False)

def sum_TopLD_variants_per_MAF_class(TopLD_directory_path, population):
	"""
	Parses TopLD variant annotation files and sums the number of variants per MAF class for a given population.

	Args:
		directory_path (str): Path to the directory containing files.
		population (str): Population ID to analyze (AFR, EAS, EUR, or SAS).
	...
	filenames = [f"{population}_chr{chr}_no_filter_0.2_1000000_info_annotation.csv.gz" for chr in chrs]
...
def count_TopLD_variants_per_LD_threshold_and_MAF_class(population, TopLD_directory_path, LD_directory_path, MAF_classes, MAF_class_sums, LD_thresholds):
	"""
	...
	chrs = [str(i) for i in range(1, 23)] + ['X']
	for chr in chrs:
		annotation_file_path = os.path.join(TopLD_directory_path, f"{population}_chr{chr}_no_filter_0.2_1000000_info_annotation.csv.gz")
		LD_file_path = os.path.join(LD_directory_path, f"{population}_chr{chr}_AGV_LD_variants.txt.gz")
