...
# run
zcat "${AGV_LD_variants_directory}EUR_${chr}_AGV_LD_variants.txt.gz" | awk '($5 >= 0.5) && ($2 ~ /^[0-9]+:[ACGT]:[ACGT]$/) && ($3 ~ /^[0-9]+:[ACGT]:[ACGT]$/) {print $2,$3,$5,$7}' OFS='\t' - > "${evaluation_directory}${chr}_R2_filtered_EUR_AGV_LD_variants.txt"
gzip "${evaluation_directory}${chr}_R2_filtered_EUR_AGV_LD_variants.txt"