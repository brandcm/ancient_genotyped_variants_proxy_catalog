...
- evaluate_variants.sh invokes evaluate_variants.py to assess many AGV-LD variant pairs are partially or completely correct in the ancient genomes. This script generates a per sample per LD threshold per chromosome mismatch file that notes when the prediction was not correct as well as a per sample overall summary file with the number of correct partial and complete matches.
